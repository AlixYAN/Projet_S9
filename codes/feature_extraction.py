...
header  = 'filename bpm bpm_weight danceability pitch_salience zcr'
n_mfcc = 5
#n_gfcc = 1
...
#        to_append = f'{filename} {np.mean(chroma_stft)} {np.mean(rmse)} {np.mean(spec_cent)} {np.mean(spec_bw)} {np.mean(rolloff)} {np.mean(zcr)} {np.mean(tempo)} {np.mean(utempo)}'    
        ...
        to_append = f'{filename} {np.mean(bpm)} {np.mean(firstPeakWeight)} {np.mean(danceability)} {np.mean(pitch_salience)} {np.mean(zcr)}'  
...
#            to_append += f' {np.mean(e)}'
        ...
        file = open('../features_csv/data_selected_3.csv', 'a', newline='')
