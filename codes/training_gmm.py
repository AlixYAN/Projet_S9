...
for nb_iter in range(10) :
    
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)
    
    taille = (0.2*np.size(data,0))
    ...
    #colors = ['navy', 'turquoise', 'darkorange', 'red', 'black', 'green', 'yellow', 'purple', 'brown', 'blue']
    ...
    for index, (name, estimator) in enumerate(estimators.items()):
        ...
        y_tot = estimator.predict(X)
    
        
        conf_mat = confusion_matrix(y_tot,y)
        # 0.2 depends on dataset : test_size variable
        ...
        if nb_iter == 0:
            ...
            for j in range(4):
                for k in range(4):
                     conf_mat_total[j][k] = (conf_mat_total[j][k]*i + conf_mat[j][k])/(i+1)
        ...
        total_acc = 100*np.trace(conf_mat)/taille
